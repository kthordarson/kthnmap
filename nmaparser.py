...
def main():
	...
	parser.add_option("-s","--session", dest="sessionname", help="sessionname")
	...
	print(scan)
	print(options)
	to_database(scan=scan, drop=options.droptables, xmlfile=options.xmlfilename, check=options.check, sessionname=options.sessionname)
